...
document.addEventListener('DOMContentLoaded', function () {
    ...
    wait_until_is_loaded()
    create_main_img_character();
});

socket.on('bought_characters',({bought_ch}) => {
    console.log('bought_characters  ',bought_ch)
    your_coins = parseInt(html_coins.innerHTML) ;
    create_shop(bought_ch)
})
function wait_until_is_loaded(){
    if (my_socket_id !== undefined){
        socket.emit('get_character',{my_socket_id});
    }else{
        setTimeout(wait_until_is_loaded,100);
    }
}


function create_main_img_character(){//character
    if (character_in_game){
        let splinted_character = character_in_game.split(' ');
        main_img.src = './images/Users/'+splinted_character[1];
        main_card.style.backgroundColor = splinted_character[0];
    }else{
        setTimeout(create_main_img_character , 100);
    }
...
function remake_prices(){
    ...
    for (let text_buy_index = 0; text_buy_index < elements.length; text_buy_index++) {
        if (elements[text_buy_index].style.display !== 'none'){
            if (parseInt(elements[text_buy_index].innerText) > your_coins){
...
function  make_new_buy_options(type){
    let elem = undefined;
    if (type === 'color'){
        elem = document.getElementById(character_in_game.split(' ')[0]);

    }else if (type === 'img'){
        elem = document.getElementById(character_in_game.split(' ')[1]);
    }else{
        create_exception('Something want wrong please refresh the page.',10,'warning');
        return
    }
    // console.log(elem,type,character_in_game.split(' '))
...
function equip(type,item){
    if (type === 'color'){
        character_in_game = item+' '+character_in_game.split(' ')[1];

    }else if (type === 'img'){
        character_in_game = character_in_game.split(' ')[0]+' '+item;
    }else{
        create_exception('Something want wrong please refresh the page.',10,'warning');
        return
    }
...
function buy(item,cost){
    console.log(item)
    your_coins -= cost
    html_coins.innerHTML = `${your_coins}`;
...
function crate_buy_equip_btn(type,what_to_buy_equip,cost,already_bought,equipped,remove_elem){
    ...
    btn_buy.className = 'btn btn-default buy_btn text-white mb10 mr20 '+(your_coins < cost?'bg-danger' : 'bg-success');
    btn_buy.innerText = 'buy';
    btn_buy.disabled = (your_coins < cost)
...
function create_img(src,cost,already_bought){
    ...
    let div_buy_option = crate_buy_equip_btn('img',src,cost,already_bought,character_in_game.includes(src),buy_text);
...
function create_color_card(color,already_bought){
    ...
    let div_buy_option = crate_buy_equip_btn('color',color[0],color[1],already_bought,character_in_game.includes(color[0]),buy_text);
...
function restart_your_img(){
    // character_in_game = '#00000000 def.png';
    set_to_equipped_html(character_in_game.split(' ')[0])
    set_to_equipped_html(character_in_game.split(' ')[1])
    character_in_game = '#00000000 def.png';
    set_to_default_equipped_html(character_in_game.split(' ')[0])
    set_to_default_equipped_html(character_in_game.split(' ')[1])
